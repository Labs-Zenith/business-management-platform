...
describe("ExpenseChartCards", () => {
  it("renders both chart cards with data", () => {
    ...
    expect(screen.queryByText("Sin gastos para graficar.")).not.toBeInTheDocument();
    expect(screen.queryByText("Sin gastos en los ultimos meses.")).not.toBeInTheDocument();
  });
  ...
  it("shows the empty state for gastos por categoria when every category total is 0", () => {
    ...
    expect(screen.getByText("Sin gastos para graficar.")).toBeInTheDocument();
  });
  ...
  it("shows the empty state for gastos por mes when no month has amount > 0", () => {
    ...
    expect(screen.getByText("Sin gastos en los ultimos meses.")).toBeInTheDocument();
  });
});
