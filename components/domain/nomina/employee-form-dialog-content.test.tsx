...
describe("EmployeeFormDialog (create mode)", () => {
  ...
  it("blocks submission client-side and shows an inline live-validation error when baseSalary is cleared (no request sent)", async () => {
    // Live validation via the shared `useZodForm` hook against
    // `lib/schemas/employee.ts`'s `employeeCreateSchema` directly (messages
    // come from the schema as-is, not a custom Spanish override) — the error
    // surfaces on BLUR (once `baseSalary` is `touched`), and the submit
    // button itself is disabled while the form is invalid, so a click on it
    // is a no-op (not the trigger for the error anymore, unlike the old
  ...
  it("enables the submit button once baseSalary is a valid positive amount", async () => {
    const user = userEvent.setup();
    const fetchMock = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({ data: { id: "new-id", name: "Nuevo Empleado" } }),
