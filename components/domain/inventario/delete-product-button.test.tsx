...
const PRODUCT_NAME = "Shampoo Profesional";

function renderButton(productActive = true) {
  return render(
    <DeleteProductButton productId={PRODUCT_ID} productName={PRODUCT_NAME} productActive={productActive} />
  );
}
...
describe("DeleteProductButton", () => {
  ...
  it("DELETEs the product and refreshes the list on success", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(`/api/products/${PRODUCT_ID}`, {
      method: "DELETE",
 ...
 */
describe("DeleteProductButton — Desactivar recovery", () => {
  const CONFLICT_BODY = {
    error: {
      code: "CONFLICT",
  ...
  it("PATCHes active:false and refreshes when Desactivar is clicked", async () => {
    ...
    expect(fetchMock).toHaveBeenLastCalledWith(`/api/products/${PRODUCT_ID}`, {
      method: "PATCH",
      headers: { "content-type": "application/json" },
