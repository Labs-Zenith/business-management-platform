...
- **11.9.1** **Dirección del movimiento según el tipo de factura:** una **nota crédito** (`nota_credito`) es una **devolución**, así que sus líneas con producto generan movimientos `in` y **suman** al inventario. Cualquier otro tipo genera `out`: la `venta` obviamente, y también la **nota débito** (`nota_debito`), que factura de *más* (un cargo extra o un envío adicional), nunca de menos. Un tipo desconocido cae por defecto en `out`, la dirección guardada.
- **11.9.2** La guarda de piso en cero (regla 11.9) **solo aplica a la dirección `out`**: sumar unidades nunca puede dejar el stock negativo, así que una nota crédito jamás se rechaza por stock insuficiente — puede registrarse incluso con 0 unidades en inventario.
- **11.9.3** Al **editar** una factura se revierte cada línea antigua y se reaplica cada línea nueva, y ambas direcciones se invierten según el tipo: una venta revierte con `in` y reaplica con `out`; una nota crédito revierte con `out` y reaplica con `in`. El lado que emite `out` es el que lleva la guarda. Por eso **editar una nota crédito puede rechazarse** si las unidades devueltas ya se revendieron: revertir la devolución exigiría sacarlas otra vez del inventario y no están.
