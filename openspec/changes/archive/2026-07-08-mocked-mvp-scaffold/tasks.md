...
## Follow-up fix (post-PR10): shared dashboard navigation shell [x] COMPLETE

Every PR from PR2 through PR10 individually deferred "no `app/(dashboard)/layout.tsx` exists yet," so the gap fell through the cracks entirely — a real user could only reach dashboard sections by typing URLs, with no clickable nav or logout affordance.

- [x] F.1 RED `components/layout/logout-button.test.tsx` + `app/(dashboard)/layout.test.tsx` → GREEN `components/layout/{nav-items.ts,logout-button.tsx,dashboard-sidebar.tsx,dashboard-bottom-nav.tsx,dashboard-topbar.tsx}` + `app/(dashboard)/layout.tsx` — shadcn/ui-based nav shell (desktop sidebar + mobile bottom nav, per `docs/ui-ux-flow.md`'s "Navegacion principal"), links to Dashboard/Clientes/Facturas/Pagos/Negocio, logout button (POSTs `/api/auth/logout`, redirects to `/login`), `requireSession()` defense-in-depth call, existing page content untouched.
- [x] F.2 Updated `e2e/smoke.spec.ts` to click the "Clientes" nav link instead of `page.goto("/customers")` for that section change, proving the shell is genuinely reachable/clickable; rest of the flow left unchanged (low-risk, already-passing e2e preserved).
- [x] F.3 Re-ran all 5 quality gates (`typecheck`, `lint`, `test`, `build`, `test:e2e`) — all green, zero regressions. 235 unit tests (up from 231), 4/4 e2e passed.
