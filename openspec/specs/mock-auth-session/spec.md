...
The system MUST expose an `AuthPort` with `getSession(): Session | null`, where `Session = { userId, businessId, email }`. All UI and service code MUST depend only on this port, never on the mock adapter directly.
...
- THEN it returns a `Session` containing the demo user's `userId`, `businessId`, and `email`
...
The mock adapter MUST authenticate the single seeded demo user via email/password and issue an httpOnly, opaque session cookie; logout MUST clear it.
