...
/**
 * `delete` is the repo's only OTHER transactional writer besides
 * `create`/`update`. These assert on the EMITTED SQL TEXT (mirroring
 * `lib/db/product-repo.test.ts#delete`'s assertions) because the ORDER and
 * CONTENT of the statements is the contract: lock first (so a concurrent
 * `invoice_items` insert cannot make the count stale), then count
 * references, then drop the product ONLY if that count was zero — with no
 * third ledger-cleanup statement, since `catalog_product_variants`/
 * `catalog_price_tiers` cascade automatically.
 */
describe("db productCatalogRepo.delete", () => {
  beforeEach(() => {
    mockSql.mockReset();
    mockTx.mockReset();
    mockRunTransaction.mockReset();
  ...
  it("runs every statement inside ONE transaction, in lock → count → delete order (no ledger statement)", async () => {
    mockTx
      .mockResolvedValueOnce([{ id: PRODUCT_ID }]) // FOR UPDATE lock
      .mockResolvedValueOnce([{ invoice_count: 0 }]) // reference count
      .mockResolvedValueOnce([]); // DELETE catalog_products

    const result = await productCatalogRepo.delete(BUSINESS_ID, PRODUCT_ID);

    expect(result).toEqual({ outcome: "deleted" });
    expect(mockRunTransaction).toHaveBeenCalledTimes(1);
    expect(mockTx).toHaveBeenCalledTimes(3);
    expect(queryTextAt(0)).toContain("FOR UPDATE");
    expect(queryTextAt(0)).toContain("FROM catalog_products");
    // `invoice_items` has no business_id of its own, hence the join.
    expect(queryTextAt(1)).toContain("COUNT(DISTINCT ii.invoice_id)");
    expect(queryTextAt(1)).toContain("ii.catalog_product_id");
    expect(queryTextAt(1)).toContain("JOIN invoices");
    expect(queryTextAt(2)).toContain("DELETE FROM catalog_products");
  });
  ...
  it("scopes the lock to the requesting business", async () => {
    ...
    await productCatalogRepo.delete(BUSINESS_ID, PRODUCT_ID);
  ...
  it("returns not_found and issues NO mutation when the lock finds nothing (unknown or cross-business id)", async () => {
    ...
    const result = await productCatalogRepo.delete(OTHER_BUSINESS_ID, PRODUCT_ID);
