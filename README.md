# Business Management Platform

Planeacion de un MVP SaaS para control interno de clientes, facturas, pagos y saldos pendientes.

El proyecto esta definido para construirse con Next.js, TypeScript, shadcn/ui, Tailwind CSS, Supabase, Vercel y una API documentada con OpenAPI/Swagger.

## Estado actual

Esta etapa contiene documentacion funcional y tecnica. Todavia no hay codigo de aplicacion implementado.

El proyecto usa Spec-Driven Development (SDD): las propuestas de cambio y las specs vivas se gestionan en [`openspec/`](openspec/), sincronizadas con memoria persistente (Engram).

## Documentacion

La documentacion completa esta en [docs/](docs/README.md).

Documentos principales:

- [Plan de producto](docs/product-plan.md)
- [Alcance MVP](docs/mvp-scope.md)
- [Arquitectura tecnica](docs/technical-architecture.md)
- [Modelo de datos](docs/database-model.md)
- [Especificacion API](docs/api-spec.md)
- [Plan de seguridad](docs/security-plan.md)
- [Flujo UI/UX](docs/ui-ux-flow.md)
- [Plan de pruebas](docs/testing-plan.md)
- [Plan de despliegue](docs/deployment-plan.md)
- [Roadmap](docs/roadmap.md)
